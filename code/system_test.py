...
def test_regressions():
    ...
    def test_uptime_monotonic():
        # P1.3: uptime is time-since-start, increasing monotonically.
        server = OpticalRadarServer(headless=True)
        captured = []
        server.ws_server.broadcast_system_status = lambda d: captured.append(d)

        server._start_time = time.time() - 5.0
        server._broadcast_system_status()
        server._start_time = time.time() - 12.0
        server._broadcast_system_status()

        assert captured[0]['uptime'] >= 5, f"uptime0={captured[0]['uptime']}"
        assert captured[1]['uptime'] >= 12, f"uptime1={captured[1]['uptime']}"
        assert captured[1]['uptime'] >= captured[0]['uptime'], "uptime not monotonic"
        return True

    check("Uptime is monotonic time-since-start (P1.3)", test_uptime_monotonic)
