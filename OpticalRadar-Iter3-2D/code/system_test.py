#!/usr/bin/env python
"""
system_test.py - Full system orchestration test (2D fork).

Verifies that all modules import correctly and the server + simulation
pipeline runs end-to-end without errors.  All position data is 2D.
"""
...
def test_protocol():
    ...
    def test_telemetry_roundtrip():
        vectors = [
            MotionVector(azimuth=45.0, elevation=0.0, intensity=200, class_id=1),
            MotionVector(azimuth=90.0, elevation=0.0, intensity=150, class_id=0),
        ]
        pkt = TelemetryPacket(
            version=VERSION,
            camera_id="cam01",
    ...
    check("Telemetry pack/unpack (2 vectors, 2D)", test_telemetry_roundtrip)
    ...
    check("Header size == 62 (uint16 vector_count and mode)", test_header_size)
...
def test_config():
    """Test config loading — 2D fork has no height_m."""
    print("\n=== Phase 3: Config ===")
    
    from common.config import Config, GPSConfig, IMUConfig, GridConfig
    ...
    def test_default_config():
        ...
        # 2D specific: no height_m
        assert not hasattr(cfg.grid, 'height_m'), "height_m should not exist in 2D fork"
        assert hasattr(cfg.grid, 'width_m')
        assert hasattr(cfg.grid, 'depth_m')
        return True
    
    check("Config: no height_m, has width_m/depth_m", test_default_config)


def test_voxel_grid():
    """Test 2D voxel grid operations."""
    print("\n=== Phase 4: 2D Grid ===")
    ...
    def test_grid_lifecycle():
        cfg = VoxelGridConfig(width_m=50, depth_m=50, resolution_m=2.0)
        grid = VoxelGrid(cfg)
        
        # Add a ray (2D direction)
        origin = np.array([0.0, 0.0])
        direction = np.array([1.0, 0.0])
        updated = grid.add_ray(origin, direction, intensity=10.0)
        assert updated > 0, f"No cells updated: {updated}"
        
        # Stats
        stats = grid.get_stats()
        assert 'hot_count_verified' in stats, f"Missing hot_count_verified in stats: {stats.keys()}"
    ...
    check("Grid lifecycle: add_ray -> get_stats -> decay -> get_detections (2D)", test_grid_lifecycle)


def test_kalman_filter():
    """Test 2D Kalman filter."""
    print("\n=== Phase 4b: 2D Kalman Filter ===")
    
    import numpy as np
    from server.tracking.kalman_filter import KalmanFilter, KalmanState
    
    def test_kalman_2d():
        kf = KalmanFilter()
        
        # Initialize with 2D position
        state = kf.initialize(np.array([10.0, 20.0]))
        assert state.x.shape == (4,), f"Expected state dim 4, got {state.x.shape}"
        assert state.P.shape == (4, 4), f"Expected P dim (4,4), got {state.P.shape}"
        
        pos = state.position
        assert len(pos) == 2, f"Expected 2D position, got {len(pos)}"
        assert np.allclose(pos, [10.0, 20.0])
        
        vel = state.velocity
        assert len(vel) == 2, f"Expected 2D velocity, got {len(vel)}"
        
        # Predict
        predicted = kf.predict(state, dt=0.1)
        assert predicted.x.shape == (4,)
        
        # Update
        measurement = np.array([10.5, 20.5])
        updated, innovation = kf.update(predicted, measurement)
        assert updated.x.shape == (4,)
        assert innovation >= 0
        
        return True
    
    check("Kalman filter: 4-state (x,y,vx,vy)", test_kalman_2d)
...
def test_tracker():
    ...
    def test_tracker_update():
        ...
        # Feed 2D detections over several frames
        for frame in range(10):
            detections = [MockDetection([50.0 + frame, 30.0])]
    ...
    check("Tracker: 10-frame update cycle (2D positions)", test_tracker_update)
...
def test_server_process_frame():
    ...
    print("\n=== Phase 8: Server Frame Processing (2D) ===")
    ...
    check("5-frame processing cycle (2D)", test_frame_cycle)


def test_simulation_to_server():
    """Test simulation node sends packets that server can unpack."""
    print("\n=== Phase 9: Simulation → Server Pipeline (2D) ===")
    ...
    def test_sim_packet_creation():
        ...
        # Manually call detect_targets
        detections = node.detect_targets()
        return True
    
    check("SimNode detect_targets (2D, no crash)", test_sim_packet_creation)


def main():
    print("=" * 60)
    print("OpticalRadar-Iter3-2D Full System Orchestration Test")
    print("=" * 60)
