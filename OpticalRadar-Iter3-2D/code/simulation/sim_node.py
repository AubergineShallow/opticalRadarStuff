"""
sim_node.py
PURPOSE: Simulated camera node for testing without real hardware (2D fork).

Targets and cameras are placed on the ground plane (z=0).
Elevation is set to 0 for all detections.
"""
...
from .sim_utils import enu_to_azel_2d, add_noise


@dataclass
class SimTarget:
    """Simulated target (2D — position/velocity in EN only)."""
    target_id: str
    position: np.ndarray   # [e, n] EN
    velocity: np.ndarray = None  # [ve, vn] m/s
    class_id: int = 0
...
class SimCameraConfig:
    """Simulated camera configuration (2D)."""
    camera_id: str
    position: np.ndarray   # [e, n] EN
    ...
    # GPS position (for protocol — altitude kept for packet compat)
    latitude: float = 0.0
    longitude: float = 0.0
    altitude: float = 0.0
...
class SimNode:
    """
    Simulated camera node (2D fork).

    Takes list of targets and simulates what a camera would see,
    sending appropriate telemetry packets.  All positions are 2D.
    """
    ...
    def update_targets(self, dt: float) -> None:
        """Update target positions based on velocity (2D)."""
        for target in self._targets:
            target.position += target.velocity * dt

    def detect_targets(self) -> List[Tuple[SimTarget, float]]:
        """
        Simulate target detection (2D — returns azimuth only).

        Returns:
            List of (target, azimuth) for visible targets
        """
        detections = []

        for target in self._targets:
            azimuth = enu_to_azel_2d(
                target.position[0], target.position[1],
                self.config.position[0], self.config.position[1]
            )
            ...
            detections.append((target, noisy_az))
    ...
    def process_frame(self) -> bool:
        ...
        vectors = []
        for target, azimuth in detections:
            vectors.append(ProtocolMotionVector(
                azimuth=azimuth,
                elevation=0.0,   # always 0 in 2D fork
                intensity=200,
...
def create_demo_simulation(
    ...
    """
    Create a demo simulation with cameras around an arena (2D).
