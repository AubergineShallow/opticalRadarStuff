"""
calibration.py
PURPOSE: Solve alignment errors with validation steps (2D fork).

NOTE: Calibration still operates in 3D quaternion space because camera
orientation is inherently 3D.  The corrected orientation is projected
to the ground plane by the RayBuilder.
"""
...
class Calibrator:
    ...
    def _calculate_residual(
        ...
        for obs in observations:
            ...
            if correction:
                # rotate_vector works in 3D; pad direction if needed
                d3 = np.zeros(3)
                d3[:len(direction)] = direction[:min(len(direction), 3)]
                d3 = np.array(rotate_vector(tuple(d3), correction))
                direction = d3
            
            # Normalize to consistent dimensionality (use min of available dims)
            ndim = min(len(direction), len(obs.target_position), len(obs.camera_position))
            d = direction[:ndim]
            target = obs.target_position[:ndim]
            camera = obs.camera_position[:ndim]
            
            # Distance from ray to target
            v = target - camera
            t = np.dot(v, d)
            closest = camera + max(0, t) * d
            distance = np.linalg.norm(target - closest)
