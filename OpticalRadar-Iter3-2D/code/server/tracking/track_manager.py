"""
track_manager.py
PURPOSE: Manage track lifecycle (creation, confirmation, deletion) — 2D fork.
"""
...
class TrackManager:
    """
    Manage track lifecycle (2D fork).
    ...
    def update_track(
        ...
        Args:
            track_id: Track ID
            measurement: Detection position [x, y]
        ...
        # NOTE: predict_track() has already been called by Tracker.update(),
        # so track.kalman_state is already the predicted state.
        # Do NOT call predict() again here — that would double-advance the state.
