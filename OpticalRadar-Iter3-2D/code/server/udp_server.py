...
class UDPServer:
    ...
    def __init__(
        ...
        Args:
            ...
            rate_limit_per_ip: Max packets per second per IP (0 = no limit)
        """
        self.port = port
        self.max_packet_size = max_packet_size
        self.authenticator = authenticator
        self.rate_limit = rate_limit_per_ip
        ...
        self._prefilter_rejected = 0
        
        # Per-IP rate limiting: {ip: [count, window_start]}
        self._ip_counters: dict = defaultdict(lambda: [0, 0.0])
    ...
    def _process_packet(
        ...
        """Process received packet data with pre-filtering."""
        sender_ip, sender_port = addr
        
        # --- Layer 1: Size check (free) ---
        if len(data) < 10 or len(data) > self.max_packet_size:
            self._prefilter_rejected += 1
            return None
        
        # --- Layer 2: Protocol version byte check (free) ---
        if data[0] != VERSION:
            self._prefilter_rejected += 1
            return None
        
        # --- Layer 3: Per-IP rate limiting (O(1)) ---
        if self.rate_limit > 0 and self._is_rate_limited(sender_ip, receive_time):
            self._rate_limited += 1
            return None
        
        # --- Layer 4: Authentication (expensive HMAC, only on plausible packets) ---
    ...
    def _is_rate_limited(self, ip: str, now: float) -> bool:
        ...
        Returns True if the IP has exceeded rate_limit packets/second.
        """
        counter = self._ip_counters[ip]
        
        # Reset window if expired (1-second window)
        if now - counter[1] >= 1.0:
            counter[0] = 1
            counter[1] = now
            return False
        
        counter[0] += 1
        return counter[0] > self.rate_limit
    ...
    def get_stats(self) -> dict:
        ...
        return {
            ...
            'prefilter_rejected': self._prefilter_rejected,
